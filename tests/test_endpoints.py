...
class TestRequestJson(TestCase):
    ...
    def test_get_parameters(self, mock_request):
        ...
        mock_request.side_effect = Session().request
        _request_json(url="http://google.com", parameters=test_parameters)
        _, kwargs = mock_request.call_args
        self.assertEqual("GET", kwargs["method"])
    ...
    def test_post_body(self, mock_request):
        data = {"apple": "pie"}
        mock_request.side_effect = Session().request
        _request_json(url="http://google.com", body=data)
        _, kwargs = mock_request.call_args
    ...
    def test_post_parameters(self, mock_request):
        ...
        _request_json(url="http://google.com", body=data, parameters=data)
        _, kwargs = mock_request.call_args
        self.assertEqual("POST", kwargs["method"])
    ...
    def test_post_headers(self, mock_request):
        mock_request.side_effect = Session().request
        data = {"apple": "pie", "orange": None}
        _request_json(url="http://google.com", body=data, headers=data)
        _, kwargs = mock_request.call_args
        self.assertEqual("POST", kwargs["method"])
...
class TestTmdbSearchMovies(TestCase):
    def test_success(self):
        ...
        self.assertSetEqual(
            expected_results_keys, set(result.get("results", [{}])[0].keys())
        )
...
class TestTvdbSeriesEpisodesQuery(TestCase):
    ...
    def test_page_valid(self):
        tvdb_series_episodes_query(self.token, LOST_TVDB_ID_SERIES, page=1)
        tvdb_series_episodes_query(
            self.token, LOST_TVDB_ID_SERIES, page=1, season=1
        )
        tvdb_series_episodes_query(
            self.token, LOST_TVDB_ID_SERIES, page=1, season=1, episode=1
        )
        with self.assertRaises(MapiNotFoundException):
            tvdb_series_episodes_query(self.token, LOST_TVDB_ID_SERIES, page=11)
        with self.assertRaises(MapiNotFoundException):
            tvdb_series_episodes_query(
                self.token, LOST_TVDB_ID_SERIES, page=2, season=1
            )
        with self.assertRaises(MapiNotFoundException):
            tvdb_series_episodes_query(
                self.token, LOST_TVDB_ID_SERIES, page=2, season=1, episode=1
            )
