...
def test_tmdb_search_movies__api_key_fail():
    with pytest.raises(MapiProviderException):
        tmdb_search_movies(JUNK_TEXT, "the goonies", cache=False)


@pytest.mark.usefixtures("tmdb_api_key")
def test_tmdb_search_movies__year_fail(tmdb_api_key):
    with pytest.raises(MapiProviderException):
        tmdb_search_movies(
...
@pytest.mark.usefixtures("tmdb_api_key")
def test_tmdb_search_movies__not_foundtmdb_api_key(tmdb_api_key):
    with pytest.raises(MapiNotFoundException):
